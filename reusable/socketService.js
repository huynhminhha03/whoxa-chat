...
function initSocket(io) {
  ...
  io.on("connection", async (socket) => {
    ...
    // Call Events ==================================================================================
    socket.on("join-call", ({ room_id, user_id, call_type }) => {
      socket.join(room_id);
      socket.to(room_id).emit("user-connected-to-call", user_id);
      emitConnectedUserList(room_id, io);
      // console.log(call_type, "call_type");
    });

    socket.on("leave-call", ({ room_id, user_id, call_type }) => {
      socket.leave(room_id);
      socket.to(room_id).emit("user-disconnected-from-call", user_id);
      emitConnectedUserList(room_id, io);
    });
