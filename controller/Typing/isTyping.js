...
const isTyping = async (io, socket, data) => {
  try {
    ...
    const other_user_ids = [...new Set(otherUsers.map((otherUser) => otherUser.user_id))];
    ...
    await Promise.all(
      other_user_ids.map(async (other_user_id) => {
        ...
        if (receiverSocket?.dataValues?.socketId) {
          ...
          const conversation_ids = userConversations.map(conversation => conversation.conversation_id);
